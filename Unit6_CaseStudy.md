...
When we try to process the whole file, we get a warning:
  data length exceeds size of matrix

How to find the line that's broken?
 - Define positively what we want
 - Define negatively what we don't want
...
Note: Highlight the full code block to get to work in RStudio
```{r}
...
mapply(surfaceSS, mac = subMacs[ rep(c(5, 1), each = 2) ],     # Note use of panels here as well
       angle = rep(c(0, 135), 2),
       data = list(data = offlineSummary))
 
par(parCur)

offlineSummary = subset(offlineSummary, mac != subMacs[2])    # Delete duplicate location here
...
Key function: Find the nearest neighbors
```{r}
findNN = function(newSignal, trainSubset) {
  diffs = apply(trainSubset[ , 4:9], 1, 
                function(x) x - newSignal)
  dists = apply(diffs, 2, function(x) sqrt(sum(x^2)) )   # function for distances
  closest = order(dists)
...
Also notice how much of a nuisance the "orientation" feature was
